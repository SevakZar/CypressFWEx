...
| :---: | :---- | :---- | :---- | :---- | :---- | :---- |
| **1** | Authentication | Valid user login | Verify user can login with valid credentials | 1\. Navigate to home page 2\. Click Sign In 3\. Enter valid email (customer@automationcamp.org) 4\. Enter valid password (welcome01) 5\. Click Login button | User is redirected to My Account / Profile page with account details visible | Easy |
| **2** | Authentication | Invalid login \- wrong password | Verify error message for invalid credentials | 1\. Navigate to login page 2\. Enter valid email (customer@automationcamp.org) 3\. Enter wrong password 4\. Click Login button | Error message displayed: "Invalid email or password" | Easy |
| **3** | Authentication | Successful user registration | Verify new user can register with valid data | 1\. Navigate to /auth/register 2\. Fill all required fields (first name, last name, DOB, address, city, state, country, phone, email, password) 3\. Submit form | Registration successful, user redirected to login page | Medium |
...
| **6** | Product | Filter products by category | Verify category filter works | 1\. Navigate to home page 2\. Select "Drills" (or "Power Tools") from the category filter 3\. Verify filtered results | Only drill/power tool products are displayed | Medium |
| **7** | Product | Sort products by price (Low to High) | Verify sorting functionality | 1\. Navigate to home page 2\. Select "Price (Low \- High)" from sort dropdown 3\. Verify product order | Products are sorted with lowest price first (e.g. Retractable Tape Measure 8m at $8.99 near the top) | Medium |
| **8** | Shopping Cart | Add product to cart as guest | Verify guest user can add product to cart | 1\. Navigate to home page 2\. Click on a product (e.g. Claw Hammer 16oz) 3\. Click "Add to Cart" 4\. Verify cart badge | Cart badge updates to show quantity "1", success toast displayed | Easy |
...
| **12** | Contact Form | Submit contact form as guest | Verify guest can submit contact form | 1\. Navigate to /contact 2\. Fill first name, last name, email 3\. Select a subject from the dropdown 4\. Enter a message (minimum 50 characters) 5\. Click Submit | Form submitted successfully, confirmation message displayed | Medium |
| **13** | User Account | View user profile | Verify user can view their profile | 1\. Login as customer@automationcamp.org 2\. Navigate to /account/profile 3\. Verify profile fields are visible | User details are displayed (name, email, phone, address, DOB) | Easy |
...
| **18** | API Testing | GET all products | Verify the products list endpoint | 1\. Send GET request to /api/products 2\. Verify response status 3\. Verify response structure | Status 200, JSON array of products each with id, name, price fields | Medium |
